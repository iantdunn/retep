...
module.exports.createReactionRolesEmbed = function (roleEmojis) {
  const embed = new EmbedBuilder()
    ...
    .setDescription(
      "React to this message to give yourself a role. In addition to being pingable, roles will unlock game-specific text channels."
    )
    .setColor("#42f5f5");
...
module.exports.createFireboardEmbed = function (
  message,
  validReactions,
  authorNickname
) {
  ...
  const embed = new EmbedBuilder()
    ...
    .setAuthor({
      name: authorNickname || message.author.displayName,
  ...
  const imageAttachment = attachmentArray.find(
...
module.exports.createTextEmbed = function (text, color) {
  return new EmbedBuilder().setDescription(text).setColor(color);
};
